...
const getAll = asyncHandler(async (req, res) => {
  const { meta, data } = await resourceService.getAll(req.query);
  sendSuccess(res, { message: messages.SUCCESS, meta, data });
});

const getById = asyncHandler(async (req, res) => {
  const data = await resourceService.getById(parseId(req.params.id));
  sendSuccess(res, { message: messages.SUCCESS, data });
});

const update = asyncHandler(async (req, res) => {
  const data = await resourceService.update(parseId(req.params.id), req.body);
  sendSuccess(res, { message: messages.SUCCESS, data });
});

const remove = asyncHandler(async (req, res) => {
  await resourceService.remove(parseId(req.params.id));
  // ...
