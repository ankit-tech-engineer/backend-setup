...
const create = asyncHandler(async (req, res) => {
  const data = await permissionService.create(req.body, req.user.tenantKey);
  sendSuccess(res, { statusCode: httpStatus.CREATED, message: messages.CREATED, data });
});

const getAll = asyncHandler(async (req, res) => {
  const { meta, data } = await permissionService.getAll(req.query, req.user.tenantKey);
  sendSuccess(res, { message: messages.SUCCESS, meta, data });
});

const getById = asyncHandler(async (req, res) => {
  const data = await permissionService.getById(parseId(req.params.id), req.user.tenantKey);
  sendSuccess(res, { message: messages.SUCCESS, data });
});

const update = asyncHandler(async (req, res) => {
  const data = await permissionService.update(parseId(req.params.id), req.body, req.user.tenantKey);
  sendSuccess(res, { message: messages.SUCCESS, data });
});

const remove = asyncHandler(async (req, res) => {
  await permissionService.remove(parseId(req.params.id), req.user.tenantKey);
  // ...
